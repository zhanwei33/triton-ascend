...
<body>
    <div class="container">
        ...
        <div class="chart-container">
            <h2 style="margin-bottom: 20px; color: #333;">📈 地址分布图 (按地址排序)</h2>
    ...
    <script>
        ...
        function drawChart() {{
            ...
            entries.forEach((entry, idx) => {{
                const x = padding.left + ((entry.address - minAddr) / addrRange) * plotWidth;
                const xKey = Math.round(x / 15); // 按15像素分组
                if (!addressGroups[xKey]) addressGroups[xKey] = [];
                addressGroups[xKey].push({{ ...entry, idx, x }});
            }});
            ...
            for (let i = 0; i <= 10; i++) {{
                ...
                // X轴标签
                const addr = Math.round(minAddr + (addrRange / 10) * i);
            ...
            for (let i = 0; i <= 5; i++) {{
                ...
                text.setAttribute('class', 'address-label');
                text.textContent = entries.length - Math.round((entries.length / 5) * i);
                svg.appendChild(text);
            }}
            ...
            yTitle.textContent = '序号 (按地址排序)';
            svg.appendChild(yTitle);

            // 绘制数据点 - 使用垂直偏移避免重叠
            Object.values(addressGroups).forEach(group => {{
                group.forEach((item, localIdx) => {{
                    const baseY = padding.top + plotHeight - ((item.idx + 1) / entries.length) * plotHeight;
                    const offsetY = (localIdx % 3) * 15 * (localIdx % 2 === 0 ? 1 : -1);
                    const y = Math.max(padding.top + 10, Math.min(padding.top + plotHeight - 10, baseY + offsetY));
                    ...
                    // 为孤立的点添加地址标签
                    if (group.length === 1 && item.idx % 3 === 0) {{
                        const label = document.createElementNS('http://www.w3.org/2000/svg', 'text');
                        label.setAttribute('x', item.x);
                        label.setAttribute('y', y - radius - 5);
