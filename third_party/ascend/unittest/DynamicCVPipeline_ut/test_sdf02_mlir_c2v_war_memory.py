...
# ----------------------------------------------------------------------------
# SDF02-TC01: float16, M=128, N=64, K=128
# Test purpose: Verify MLIR generation of C2V dependency + WAR memory dependency under float16
# ----------------------------------------------------------------------------
@triton.jit
...
# ----------------------------------------------------------------------------
# SDF02-TC02: float32, M=128, N=64, K=128
# Test purpose: Verify MLIR generation of C2V dependency + WAR memory dependency under float32
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_sdf02_signature(dtype_str):
    """Build the argument type signature for the SDF02 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf02_tc01():
    """SDF02-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf02_tc01_c2v_war kernel to MLIR
...
def test_sdf02_tc02():
    """SDF02-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf02_tc02_c2v_war kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
