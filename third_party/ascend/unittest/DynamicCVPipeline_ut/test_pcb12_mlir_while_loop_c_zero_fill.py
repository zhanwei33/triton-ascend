...
# ----------------------------------------------------------------------------
# PCB12-TC01: float16, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of while-loop A*B+C (C=0 fill) under float16
# ----------------------------------------------------------------------------
@triton.jit
...
# ----------------------------------------------------------------------------
# PCB12-TC02: float32, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of while-loop A*B+C (C=0 fill) under float32
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_pcb12_signature(dtype_str):
    """Build the argument type signature for the PCB12 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "out_ptr": f"*{dtype_str}",
...
def test_pcb12_tc01():
    """PCB12-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb12_tc01_while_matmul_fill kernel to MLIR
...
def test_pcb12_tc02():
    """PCB12-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb12_tc02_while_matmul_fill kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
