"""
Test Case: SDF16 - 3-layer nested, inner C depends on outer V (V2C cross-layer)

[MLIR Validation] Refactored version

Description: 3-layer nested (L, P, K). Outer V (tl.max) feeds into inner C via V2C.
...
# ----------------------------------------------------------------------------
# SDF16-TC01: float16, M=128, N=64, K=32, L=3, P=2
# Test purpose: Verify MLIR generation of 3-layer nesting, inner C depends on outer V (V2C cross-layer) under float16
# ----------------------------------------------------------------------------
@triton.jit
    ...
    for i in range(L):
        ...
        for j in range(P):
            for k in range(K):
                ...
                inner_cube = tl.dot(e[:, None], outer_vec.to(tl.float16)[None, None])
...
def _build_sdf16_signature(dtype_str):
    """Build the argument type signature for the SDF16 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf16_tc01():
    """SDF16-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf16_tc01_inner_c_dep_outer_v kernel to MLIR
    ...
    """
    signature = _build_sdf16_signature("fp16")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32, "BLOCK_SIZE_L": 3, "BLOCK_SIZE_P": 2}

    mlir = compile_kernel(sdf16_tc01_inner_c_dep_outer_v, signature, constants)
    _write_mlir_to_file(mlir, "sdf16_tc01_inner_c_dep_outer_v.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @sdf16_tc01_inner_c_dep_outer_v(" in mlir, \
...
if __name__ == "__main__":
    test_sdf16_tc01()
    print("All SDF16 v3 MLIR validation tests passed!")
