"""
Test Case: PCB13 - while loop, A*B+C where C!=0

[MLIR Validation] Refactored version

Description: while loop, A*B+C where C!=0
...
Test Cases:
  ...
  - PCB13-TC02: float32, M=128, N=64, K=32
"""
...
# ----------------------------------------------------------------------------
# PCB13-TC01: float16, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of while-loop A*B+C (C!=0) under float16
# ----------------------------------------------------------------------------
@triton.jit
def pcb13_tc01_while_matmul_add(
    ...
    c = tl.load(c_ptr + offs_n * stride_cn, mask=offs_n < N, other=0.0)  # (N,)
    ...
    while k < K:
        ...
        result = dot_result + c[None, :]  # (K,N) + (1,N) -> (K,N)
...
# ----------------------------------------------------------------------------
# PCB13-TC02: float32, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of while-loop A*B+C (C!=0) under float32
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_pcb13_signature(dtype_str):
    """Build the argument type signature for the PCB13 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_pcb13_tc01():
    """PCB13-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb13_tc01_while_matmul_add kernel to MLIR
    ...
    """
    signature = _build_pcb13_signature("fp16")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb13_tc01_while_matmul_add, signature, constants)
    _write_mlir_to_file(mlir, "pcb13_tc01_while_matmul_add.mlir")
...
def test_pcb13_tc02():
    """PCB13-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb13_tc02_while_matmul_add kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
    signature = _build_pcb13_signature("fp32")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb13_tc02_while_matmul_add, signature, constants)
    _write_mlir_to_file(mlir, "pcb13_tc02_while_matmul_add.mlir")
...
if __name__ == "__main__":
    test_pcb13_tc01()
    test_pcb13_tc02()
    print("All PCB13 v3 MLIR validation tests passed!")
