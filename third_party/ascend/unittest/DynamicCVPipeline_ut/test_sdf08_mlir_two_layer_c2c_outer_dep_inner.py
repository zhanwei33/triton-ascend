...
# ----------------------------------------------------------------------------
# SDF08-TC01: float16, M=128, N=64, K=32, L=3
# Test purpose: Verify MLIR generation of 2-layer nesting C2C cross-layer dependency (inner CV -> outer CV) under float16
# ----------------------------------------------------------------------------
@triton.jit
...
# ----------------------------------------------------------------------------
# SDF08-TC02: float32, M=128, N=64, K=32, L=3
# Test purpose: Verify MLIR generation of 2-layer nesting C2C cross-layer dependency (inner CV -> outer CV) under float32
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_sdf08_signature(dtype_str):
    """Build the argument type signature for the SDF08 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf08_tc01():
    """SDF08-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf08_tc01_c2c_cross_layer kernel to MLIR
...
def test_sdf08_tc02():
    """SDF08-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf08_tc02_c2c_cross_layer kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
