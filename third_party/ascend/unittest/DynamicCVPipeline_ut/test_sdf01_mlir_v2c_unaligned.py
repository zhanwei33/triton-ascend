"""
Test Case: SDF01 - V2C dependency, not satisfying 32B alignment (N=3)

[MLIR Validation] Refactored version

Description: V2C dependency, not satisfying 32B alignment (N=3)
...
Test Cases:
  ...
  - SDF01-TC02: float32, M=128, N=3, K=32
"""
...
# ----------------------------------------------------------------------------
# SDF01-TC01: float16, M=128, N=3, K=32
# Test purpose: Verify MLIR generation of V2C dependency, non-32B aligned (N=3) under float16
# ----------------------------------------------------------------------------
@triton.jit
def sdf01_tc01_v2c_unaligned(
    ...
    offs_k = tl.arange(0, BLOCK_SIZE_K)  # (K,) = (32,)
    offs_n = tl.arange(0, BLOCK_SIZE_N)  # (N,) = (3,)

    c = tl.load(c_ptr + offs_n * stride_c, mask=offs_n < N, other=0.0)  # (N,) = (3,)
    vec_result = c * 2.0  # (N,) = (3,)

    for k in range(K):
        a = tl.load(a_ptr + k * stride_am + offs_k * stride_ak, mask=offs_k < K, other=0.0)  # (K,) = (32,)
        b = tl.load(b_ptr + k * stride_bk + offs_n * stride_bn, mask=offs_n < N, other=0.0)  # (N,) = (3,)
        cube_result = tl.dot(a[:, None], b[None, :]) + vec_result  # (K,1)@(1,N)+(N,) = (K,N)+(1,N) = (K,N) = (32,3)
        out_ptrs = out_ptr + offs_k[:, None] * stride_outm + offs_n[None, :] * stride_outn  # (K,N)
        out_mask = (offs_k[:, None] < K) & (offs_n[None, :] < N)  # (K,N)
        tl.store(out_ptrs, cube_result, mask=out_mask)


# ----------------------------------------------------------------------------
# SDF01-TC02: float32, M=128, N=3, K=32
# Test purpose: Verify MLIR generation of V2C dependency, non-32B aligned (N=3) under float32
# ----------------------------------------------------------------------------
@triton.jit
def sdf01_tc02_v2c_unaligned(
    ...
    for k in range(K):
        a = tl.load(a_ptr + k * stride_am + offs_k * stride_ak, mask=offs_k < K, other=0.0)  # (K,) = (32,)
        b = tl.load(b_ptr + k * stride_bk + offs_n * stride_bn, mask=offs_n < N, other=0.0)  # (N,) = (3,)
        cube_result = tl.dot(a[:, None], b[None, :]) + vec_result  # (K,N) = (32,3)
...
def _build_sdf01_signature(dtype_str):
    """Build the argument type signature for the SDF01 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf01_tc01():
    """SDF01-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf01_tc01_v2c_unaligned kernel to MLIR
    ...
    """
    signature = _build_sdf01_signature("fp16")
    constants = {"BLOCK_SIZE_N": 3, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(sdf01_tc01_v2c_unaligned, signature, constants)
    _write_mlir_to_file(mlir, "sdf01_tc01_v2c_unaligned.mlir")
...
def test_sdf01_tc02():
    """SDF01-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf01_tc02_v2c_unaligned kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
    signature = _build_sdf01_signature("fp32")
    constants = {"BLOCK_SIZE_N": 3, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(sdf01_tc02_v2c_unaligned, signature, constants)
    _write_mlir_to_file(mlir, "sdf01_tc02_v2c_unaligned.mlir")
...
if __name__ == "__main__":
    test_sdf01_tc01()
    test_sdf01_tc02()
    print("All SDF01 v3 MLIR validation tests passed!")
