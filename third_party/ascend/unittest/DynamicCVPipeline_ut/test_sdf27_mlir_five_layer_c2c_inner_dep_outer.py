"""
Test Case: SDF27 - 5-layer inner C depends outer C

[MLIR Validation] Refactored version

Description: 5-layer nested (R, Q, L, P, K). Inner loop uses l1c from outer in tl.dot.
...
# ----------------------------------------------------------------------------
# SDF27: 5-layer inner C depends outer C
# Test purpose: Verify MLIR generation of 5-layer nesting, inner C depends on outer C under float16
# ----------------------------------------------------------------------------
@triton.jit
def sdf27(
    a_ptr,
    ...
    pid = tl.program_id(0)
    offs_r, offs_k, offs_n = tl.arange(0, BLOCK_SIZE_R), tl.arange(0, BLOCK_SIZE_K), tl.arange(0, BLOCK_SIZE_N)
    for l1 in range(R):
        a = tl.load(a_ptr + l1 * stride_am + offs_k * stride_ar, mask=offs_k < R, other=0.0)
        b = tl.load(b_ptr + l1 * stride_br + offs_k * stride_bn, mask=offs_k < R, other=0.0)
            ...
            for l3 in range(L):
                for l4 in range(P):
                    for l5 in range(K):
                        ...
                        ic = tl.dot(l1c.to(tl.float16), e[:, None])
...
def _build_sdf27_signature(dtype_str):
    """Build the argument type signature for the SDF27 kernel.

    Note: In SDF27, K is a constexpr parameter and does not appear in signature.
    """
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
...
def test_sdf27():
    """SDF27: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf27 kernel to MLIR
    ...
    """
    signature = _build_sdf27_signature("fp16")
    constants = {
        "K": 32,
        "BLOCK_SIZE_K": 32,
        "BLOCK_SIZE_N": 64,
    ...
    mlir = compile_kernel(sdf27, signature, constants)
    _write_mlir_to_file(mlir, "sdf27.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @sdf27(" in mlir, \
...
if __name__ == "__main__":
    test_sdf27()
    print("All SDF27 v3 MLIR validation tests passed!")
