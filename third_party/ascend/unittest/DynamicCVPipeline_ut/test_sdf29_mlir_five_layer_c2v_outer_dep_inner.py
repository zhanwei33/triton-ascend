...
# Test purpose: Verify MLIR generation of 5-layer nesting, outer V depends on inner C accumulation (C2V cross-layer) under float16
# ----------------------------------------------------------------------------
@triton.jit
def sdf29(
    a_ptr,
...
def _build_sdf29_signature(dtype_str):
    """Build the argument type signature for the SDF29 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf29():
    """SDF29: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf29 kernel to MLIR
