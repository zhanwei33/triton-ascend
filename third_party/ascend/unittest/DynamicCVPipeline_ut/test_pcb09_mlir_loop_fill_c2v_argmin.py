"""
Test Case: PCB09 - for loop, Cube with fill upstream + C2V dependency (tl.argmin)

[MLIR Validation] Refactored version

Description: for loop, Cube with fill upstream + C2V dependency (tl.argmin)
...
Test Cases:
  ...
  - PCB09-TC02: float32, M=128, N=64, K=32
"""
...
# ----------------------------------------------------------------------------
# PCB09-TC01: float16, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of Cube with fill upstream + C2V dependency (tl.argmin) under float16
# ----------------------------------------------------------------------------
@triton.jit
def pcb09_tc01_kernel(
    ...
    for k in range(K):
        ...
        vec_result = tl.argmin(tl.ravel(cube_result + c), axis=0)  # (K,N)+(N,)
        tl.store(out_ptr + pid * stride_out, vec_result)


# ----------------------------------------------------------------------------
# PCB09-TC02: float32, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of Cube with fill upstream + C2V dependency (tl.argmin) under float32
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_pcb09_signature(dtype_str):
    """Build the argument type signature for the PCB09 kernel.

    Note: out_ptr stores the argmin result with type i64.
    """
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
...
def test_pcb09_tc01():
    """PCB09-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb09_tc01_kernel kernel to MLIR
    ...
    """
    signature = _build_pcb09_signature("fp16")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb09_tc01_kernel, signature, constants)
    _write_mlir_to_file(mlir, "pcb09_tc01_kernel.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @pcb09_tc01_kernel(" in mlir, \
...
def test_pcb09_tc02():
    """PCB09-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb09_tc02_kernel kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
    signature = _build_pcb09_signature("fp32")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb09_tc02_kernel, signature, constants)
    _write_mlir_to_file(mlir, "pcb09_tc02_kernel.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @pcb09_tc02_kernel(" in mlir, \
        "Kernel function definition not found in MLIR code"
    assert "scope" in mlir, "MLIR code does not contain the 'scope' keyword"
...
if __name__ == "__main__":
    test_pcb09_tc01()
    test_pcb09_tc02()
    print("All PCB09 v3 MLIR validation tests passed!")
