...
# ----------------------------------------------------------------------------
# SDF11-TC01: float16, M=128, N=64, K=32, L=3
# Test purpose: Verify MLIR generation of 2-layer nesting, outer V depends on inner C (C2V cross-layer) under float16
# ----------------------------------------------------------------------------
@triton.jit
...
# ----------------------------------------------------------------------------
# SDF11-TC02: float32, M=128, N=64, K=32, L=3
# Test purpose: Verify MLIR generation of 2-layer nesting, outer V depends on inner C (C2V cross-layer) under float32
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_sdf11_signature(dtype_str):
    """Build the argument type signature for the SDF11 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf11_tc01():
    """SDF11-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf11_tc01_nested_c2v kernel to MLIR
...
def test_sdf11_tc02():
    """SDF11-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf11_tc02_nested_c2v kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
