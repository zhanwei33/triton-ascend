"""
Test Case: SDF07 - 2-layer nested, only outer has CV, inner has scalar only

[MLIR Validation] Refactored version

Description: 2-layer nested, only outer has CV, inner has scalar only.
Outer loop over L with CV (tl.dot + vector divide), inner loop over K (scalar k+i).
...
Test Cases:
  - SDF07-TC01: float16, M=128, N=64, K=32, L=3
  - SDF07-TC02: float32, M=128, N=64, K=32, L=3
"""
...
# ----------------------------------------------------------------------------
# SDF07-TC01: float16, M=128, N=64, K=32, L=3
# Test purpose: Verify MLIR generation of 2-layer nesting, outer CV + inner scalar under float16
# ----------------------------------------------------------------------------
@triton.jit
def sdf07_tc01_outer_cv_only(
    a_ptr,
    b_ptr,
    c_ptr,
    d_ptr,
    ...
    for i in range(L):
        ...
        c = tl.load(c_ptr + offs_l * stride_c, mask=offs_l < L, other=0.0)  # (L,)
        d = tl.load(d_ptr + offs_l * stride_d, mask=offs_l < L, other=0.0)  # (L,)
        outer_vec = c / (d + 1e-6)  # (L,)

        out_ptrs = out_ptr + offs_l[:, None] * stride_out + offs_l[None, :]
        tl.store(out_ptrs, outer_cube + outer_vec[:, None], mask=offs_l[:, None] < L)
...
# ----------------------------------------------------------------------------
# SDF07-TC02: float32, M=128, N=64, K=32, L=3
# Test purpose: Verify MLIR generation of 2-layer nesting, outer CV + inner scalar under float32
# ----------------------------------------------------------------------------
@triton.jit
def sdf07_tc02_outer_cv_only(
    a_ptr,
    b_ptr,
    c_ptr,
    d_ptr,
...
def _build_sdf07_signature(dtype_str):
    """Build the argument type signature for the SDF07 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf07_tc01():
    """SDF07-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf07_tc01_outer_cv_only kernel to MLIR
    ...
    """
    signature = _build_sdf07_signature("fp16")
    constants = {"BLOCK_SIZE_K": 32, "BLOCK_SIZE_L": 3}

    mlir = compile_kernel(sdf07_tc01_outer_cv_only, signature, constants)
    _write_mlir_to_file(mlir, "sdf07_tc01_outer_cv_only.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @sdf07_tc01_outer_cv_only(" in mlir, \
...
def test_sdf07_tc02():
    """SDF07-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf07_tc02_outer_cv_only kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
    signature = _build_sdf07_signature("fp32")
    constants = {"BLOCK_SIZE_K": 32, "BLOCK_SIZE_L": 3}

    mlir = compile_kernel(sdf07_tc02_outer_cv_only, signature, constants)
    _write_mlir_to_file(mlir, "sdf07_tc02_outer_cv_only.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @sdf07_tc02_outer_cv_only(" in mlir, \
        "Kernel function definition not found in MLIR code"
    assert "scope" in mlir, "MLIR code does not contain the 'scope' keyword"
...
if __name__ == "__main__":
    test_sdf07_tc01()
    test_sdf07_tc02()
    print("All SDF07 v3 MLIR validation tests passed!")
