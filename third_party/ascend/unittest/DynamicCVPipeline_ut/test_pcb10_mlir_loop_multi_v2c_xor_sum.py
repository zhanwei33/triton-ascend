"""
Test Case: PCB10 - for loop, multiple V2C dependency (tl.xor_sum + add -> Cube)

[MLIR Validation] Refactored version

Description: for loop, multiple V2C dependency (tl.xor_sum + add -> Cube)
...
Test Cases:
  ...
  - PCB10-TC02: float32, M=128, N=64, K=32
"""
...
# ----------------------------------------------------------------------------
# PCB10-TC01: float16, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of multiple V2C dependency (tl.xor_sum + add -> Cube) under float16
# ----------------------------------------------------------------------------
@triton.jit
def pcb10_tc01_kernel(
    ...
    for k in range(K):
        ...
        c_int = c.to(tl.int32)  # xor_sum requires integer type
        vec1_result = tl.xor_sum(c_int)  # scalar
        ...
        cube_result = tl.dot(a[:, None], b[None, :]) + vec1_result + vec2_result  # (K,N) + scalar + (N,)
        out_ptrs = out_ptr + offs_k[:, None] * stride_out + offs_n[None, :]
        tl.store(out_ptrs, cube_result, mask=(offs_k[:, None] < K) & (offs_n[None, :] < N))


# ----------------------------------------------------------------------------
# PCB10-TC02: float32, M=128, N=64, K=32
# Test purpose: Verify MLIR generation of multiple V2C dependency (tl.xor_sum + add -> Cube) under float32
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_pcb10_signature(dtype_str):
    """Build the argument type signature for the PCB10 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_pcb10_tc01():
    """PCB10-TC01: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb10_tc01_kernel kernel to MLIR
    ...
    """
    signature = _build_pcb10_signature("fp16")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb10_tc01_kernel, signature, constants)
    _write_mlir_to_file(mlir, "pcb10_tc01_kernel.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @pcb10_tc01_kernel(" in mlir, \
...
def test_pcb10_tc02():
    """PCB10-TC02: Verify float32 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile pcb10_tc02_kernel kernel to MLIR
      2. Verify MLIR code is successfully generated and non-empty
      3. Verify MLIR code contains the function definition
      4. Verify MLIR code contains the "scope" keyword
    """
    signature = _build_pcb10_signature("fp32")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb10_tc02_kernel, signature, constants)
    _write_mlir_to_file(mlir, "pcb10_tc02_kernel.mlir")

    assert mlir and len(mlir) > 0, "MLIR code generation failed or is empty"
    assert "func.func @pcb10_tc02_kernel(" in mlir, \
        "Kernel function definition not found in MLIR code"
    assert "scope" in mlir, "MLIR code does not contain the 'scope' keyword"
...
if __name__ == "__main__":
    test_pcb10_tc01()
    test_pcb10_tc02()
    print("All PCB10 v3 MLIR validation tests passed!")
